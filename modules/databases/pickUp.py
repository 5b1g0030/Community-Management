...
class PickUp:
    ...
    async def get_available_locker(self):
    ...
    # ====== 自動分配櫃號並登記包裹 ======
    # 【修改】函式宣告加上 async 轉換為非同步函式
    async def register_package(self, recipient_name):
        # 【修改】加上 await
        conn, cursor = await self.db_manager.get_db_connection()
        try:
            # 取得可用櫃號
            # 【修改】呼叫類別內的非同步函式需加上 await
            locker_number = await self.get_available_locker()
    ...
    # ===== 根據住戶名稱查詢櫃號 =====
    # 【修改】函式宣告加上 async 轉換為非同步函式
    async def get_locker_by_name(self, name):
        # 【修改】加上 await
        conn, cursor = await self.db_manager.get_db_connection()
        try:
            # 根據名字查詢，且確保是占用狀態
    ...
    # ===== 清除櫃位資訊 ======
    # 【修改】函式宣告加上 async 轉換為非同步函式
    async def clear_locker(self, locker_number):
        # 【修改】加上 await
        conn, cursor = await self.db_manager.get_db_connection()
        try:
            # 更新櫃位狀態，重置為「未占用」
    ...
    # ===== 取得所有櫃位狀態 =====
    # 【修改】函式宣告加上 async 轉換為非同步函式
    async def get_all_lockers_status(self):
        # 【修改】加上 await
        conn, cursor = await self.db_manager.get_db_connection()
        try:
            # 搜尋所有櫃位資料
            ...
            await cursor.execute("""
                ...
                ORDER BY locker_number ASC
            """)
            # 【修改】取得多筆資料 (fetchall) 在 aiosqlite 中為非同步操作，需加上 await
