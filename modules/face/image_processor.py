...
import io
import asyncio  # [修改]：引入 asyncio 來處理執行緒調度
...
# ===== 圖片轉RGB三通道 =====
# [修改]：將函式改為 async def，並使用 asyncio.to_thread 將 CPU 密集運算與檔案 I/O 丟到背景執行
async def changeRGB(input_data):
    """
    ...
    # [修改]：將原本的同步阻塞處理邏輯包裝成內部函式，以便交給獨立執行緒處理
    def _process_image():
        # 判斷輸入類型
        if isinstance(input_data, str):
            log.info("input_data 是字串")
            # 輸入是檔案路徑 (此處 Image.open 屬於阻塞 I/O)
            ...
            # 輸入是 numpy array (OpenCV frame, BGR 格式，此為 CPU 密集運算)
    ...
    # [修改]：使用 await 讓出控制權，直到背景執行緒完成轉換
    return await asyncio.to_thread(_process_image)

# ===== 獲取資料夾內的圖片檔案 =====
# [修改]：將函式改為 async def，避免 os.listdir 阻塞事件迴圈
async def getFiles(folder_path):
    """掃描資料夾內的圖片檔案"""
    files = []
    # [修改]：將檔案系統的 I/O 操作丟到執行緒中等待
...
# ===== 驗證圖片檔案格式 =====
# [備註]：純字串判斷，無 I/O 或 CPU 密集運算，可保持原本的同步函式，無需加上 async
...
# [修改]：改為 async def 處理非同步上傳機制與耗時的圖片儲存
async def process_uploaded_image(image_file, temp_path):
    try:       
        # [修改]：在 Quart 中，request.files 物件的 read() 必須使用 await 來非同步讀取
        image_data = await image_file.read()
        # print(f"[測試辨識] 讀取圖片資料，大小：{len(image_data)} bytes")
                    
        # [修改]：將 CPU 密集的圖片調整及 I/O 密集的圖片儲存包裝為內部函式
        ...
        # [修改]：交由 asyncio.to_thread 執行，避免卡住伺服器
        img = await asyncio.to_thread(_resize_and_save)
